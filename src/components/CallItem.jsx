...
import { formatPhoneNumber, getCallIcon, getCallDescription } from '../utils/callHelpers.jsx';
import { motion, AnimatePresence } from 'framer-motion';

function CallItem({ call, onToggleArchive }) {
  const [isHovered, setIsHovered] = useState(false);
  const { id, call_type, direction, duration, from, to, via, created_at, is_archived } = call;
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-medium text-gray-900">
                {formatPhoneNumber(direction === 'inbound' ? from : to)}
              </span>
              <span className="text-xs text-gray-500 font-light">
                {getCallDescription(call_type, direction, duration)} via {formatPhoneNumber(via)}
              </span>
            </div>
          </div>
          <div className="text-xs text-gray-500 flex items-center">
            {isHovered && (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setTimeout(() => onToggleArchive(id, !is_archived), 50);
                }}
                aria-label={is_archived ? "Move to inbox" : "Archive call"}
              >
                {is_archived ? <Inbox size={16} /> : <Archive size={16} />}
              </button>
            )}
